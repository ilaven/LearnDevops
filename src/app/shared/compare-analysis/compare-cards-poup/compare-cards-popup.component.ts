...
@Component({
  selector: 'compare-cards-popup',
  templateUrl: './compare-cards-popup.component.html',
  standalone: false,
})
export class CompareCardsPopupComponent implements OnInit {
  ...
  eventName: any;
  themeoption: any;
  constructor(public dialogRef: MatDialogRef<CompareCardsPopupComponent>, private mlForm: FormBuilder, private zone: NgZone,
  ...
  async loadAC(param: any) {
    var tempAcVal = param;
    this.eventName = tempAcVal.cardName;
    ...
    this.zone.runOutsideAngular(() => {
      ...
      if (Object.keys(tempAcVal).length > 0) {
        if (tempAcVal.UPSFedExVal != undefined) {
          chart.data = [{
            "category": "UPS",
            "Count": tempAcVal.upsVal,
            "color": "#dc9d3c"
          }, {
            "category": "FedEx",
            "Count": tempAcVal.FedexVal,
            "color": "#8250e7"
          }, {
            "category": "UPS-FedEx",
            "Count": tempAcVal.UPSFedExVal,
            "color": "#059fd1"
          }];
        }
        else {
      ...
      if (this.eventName != "Packages Shipped") {

...
@Component({
  selector: 'app-usps-schedular-edit',
...
export class SchedularEditComponent implements OnInit {
  ...
  btn_submit_click(): void {
    ...
    if (this.btnEdit === 'yes') {
      ...
      this.httpUspsService.updateReportScheduler(t007reportLogObj).subscribe({
        next: (result) => {
      ...
      this.httpUspsService.addMailId(t007reportLogObj).subscribe({
        next: (result) => {
          if (result) {
            this.openModal(

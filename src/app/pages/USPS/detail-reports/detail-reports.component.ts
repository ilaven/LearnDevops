...
import { UspsReportLogComponent } from './report-log/report-log.component';
import { AlertPopupComponent } from 'src/app/shared/alert-popup/alert-popup.component';
import { MailPopupComponent } from './mail-popup/mail-popup.component';
import { map, startWith } from 'rxjs/operators';

@Component({
  selector: 'app-usps-detail-reports',
  templateUrl: './detail-reports.component.html',
  styleUrls: ['./detail-reports.component.scss'],
  standalone: false
})
export class UspsDetailReportsComponent implements OnInit, AfterViewInit {
    ...
    try {
      ...
      await this.commonService.getUserprofileData().then(
        (result: any) => {
          ...
          this.detailReportsFormGroup.get('uspsClientId')?.setValue(this.clientID);
  ...
  collection(isInitial = false): void {
    if (!isInitial) this.openLoading();
    var t001ClientObj = {};
    this.httpUspsService.fetchReportDescription(t001ClientObj).subscribe({
      next: (result: any) => this.ReportDescriptionResult(result),
      error: (err: any) => { this.closeLoading(); console.error(err) }
    });
  }

  ReportDescriptionResult(result: any[]): void {
    this.itemsAC = [];
    this.tempMasterAC = [];
  ...
  async reportLog_clickHandler() {
    ...
    await this.httpClientService.fetchReportLog(userprofile).subscribe({
      next: (result: any[]) => {
        ...
        const dialogRef = this.dialog.open(UspsReportLogComponent, {
          width: '95%',
          // height: '50%',
          // panelClass: this.panelClass,
  ...
  radiobtn_clickHandler(value: string, reportName: string, event: Event): void {
    ...
    if (this.selectedReport == 'INVOICE_DATA') {
      ...
      this.detailReportsFormGroup.get('reportFormat')?.setValue('Standardized Data');
    } else {
      this.isExcelVisible = true;
      this.strippedDataVisible = false;
  ...
  async runreport_clickHandler(): Promise<void> {
    ...
    if (reportsselectedValue != null && reportsselectedValue != undefined && reportsselectedValue != "") {
      if (pdf_btnselected == "" || pdf_btnselected == null || pdf_btnselected == undefined) {
        ...
        if (reportsselectedValue == 'INVOICE_DATA' || reportsselectedValue == 'CHARGE_DESC_SUMMARY' || reportsselectedValue == 'INVOICE_SUMMARY')
      ...
      else if (pdf_btnselected == 'Standardized Data') {

        if (reportsselectedValue == "INVOICE_DATA") {
          this.report_clickHandler(null);
        }
        else {
          this.openModal("Standardized Data Not Available for this Report");
          return;
        }
      }
      else {
  ...
  report_clickHandler(event: any): void {
    ...
    if (pdf_btnselected == 'Standardized Data') {
      this.detailReportsFormGroup.get('reportType')?.setValue("STANDARIZED_DATA");
      this.detailReportsFormGroup.get('designFileName')?.setValue("STANDARIZED_DATA");
      this.detailReportsFormGroup.get('reportName')?.setValue("STANDARIZED DATA");
    }
    else {
    ...
    if (this.detailReportsFormGroup.get('reportType')?.value == "STANDARIZED_DATA" || this.detailReportsFormGroup.get('reportType')?.value == "INVOICE_DATA") {
      ...
      if (this.clientName == "admin") {
        ...
        this.httpUspsService.runReport(this.detailReportsFormGroup.value).subscribe({
          next: (result) => {

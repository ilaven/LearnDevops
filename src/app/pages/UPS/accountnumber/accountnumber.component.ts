...
@Component({
  selector: 'app-ups-accountnumber',
  templateUrl: './accountnumber.component.html',
  styleUrls: ['./accountnumber.component.scss'],
  standalone: false
})
export class UpsAccountnumberComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async fetchaccountDetails(clientIdUps: any) {
    ...
    try {
      const result = await firstValueFrom(
        this.httpClientService.fetchaccountDetailsUPS(this.t002ClntObjUps)
      );
  ...
  SortOrder_Update_ClickHandlerUPS() {
    this.httpClientService.insertOrUpdateAccountSortOrder(this.accountAC).subscribe({
      next: (result: any) => {
        if (result == true)
  ...
  accountUpdate_ClickHandler(rowObj: any, index: any) {
    var updateObj = {};
    updateObj = rowObj;
    this.httpClientService.updateAccountNumber(updateObj).subscribe({
      next: (result: any) => {

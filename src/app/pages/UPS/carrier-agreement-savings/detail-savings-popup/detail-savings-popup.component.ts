...
@Component({
  selector: 'app-ups-detail-savings-popup',
...
export class DetailSavingsPopup implements OnInit, AfterViewInit, OnDestroy {
  ...
  async savePDF(): Promise<void> {
    ...
    //   var clientImageDataUrl = await this.getBase64ImageFromURL(
    //    'https://cors-anywhere.herokuapp.com/' + this.ImageUrlData
    //    // this.imageUrl + this.clientNameRegex + ".jpg"
    //  );
      ...
      if (this.clientType().toUpperCase() === 'UPS') {
        docDefinition = {
          pageSize: { width: 700, height: 'auto' },
          ...
          content: [
            ...
            },
            {
              table: {
                ...
                body: [
                  [
                    { text: 'Carrier', alignment: 'left', style: 'tableHeader' },
                    { text: 'Description', alignment: 'left', style: 'tableHeader' },
                    { text: 'Invoice Period', alignment: 'left', style: 'tableHeader' },
                    { text: 'Total Savings', alignment: 'left', style: 'tableHeader' },
                    { text: 'YTD Savings', alignment: 'left', style: 'tableHeader' }
                  ],
                  ...this.dataProviderList.map((p: any) => ([
                    { text: p.carrier, alignment: 'left', style: 'body' },
                    { text: p.description, alignment: 'left', style: 'body' },
                    { text: p.date, alignment: 'left', style: 'body' },
                    { text: '$' + this.setCommaQty(Number(String(p.qty).replace('$', '').replace(/[,]/g, '').replace('%', '')).toFixed(2)), alignment: 'left', style: 'body' },
                    { text: '$' + this.setCommaQty(Number(String(p.rate).replace('$', '').replace(/[,]/g, '').replace('%', '')).toFixed(2)), alignment: 'left', style: 'body' }
                  ]))
                ]
              }
            },
              ...
              table: {
                ...
                body: [
                  ...
                  ...this.dataProviderList.map((p: any) => ([
                    { text: p.carrier, alignment: 'left', style: 'body' },
                    { text: p.description, alignment: 'left', style: 'body' },
                    { text: p.contractDate, alignment: 'left', style: 'body' },
                    { text: '$' + this.setCommaQty(Number(String(p.qty).replace('$', '').replace(/[,]/g, '').replace('%', '')).toFixed(2)), alignment: 'left', style: 'body' },
                    { text: '$' + this.setCommaQty(Number(String(p.rate).replace('$', '').replace(/[,]/g, '').replace('%', '')).toFixed(2)), alignment: 'left', style: 'body' }
                  ]))
                ]

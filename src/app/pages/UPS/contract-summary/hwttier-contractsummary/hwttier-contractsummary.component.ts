...
@Component({
  selector: 'app-hwttier-contractsummary',
  templateUrl: './hwttier-contractsummary.component.html',
  styleUrls: ['./hwttier-contractsummary.component.css'],
  standalone: false
})
export class HundredweightTierComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<HundredweightTierComponent>, public dialog: MatDialog, private loaderService: LoaderService,
  ...
  async getData() {
    ...
    if (this.carrier.toLowerCase() == 'fedex') {
      ...
      var hwtData = await this.fedexService.fetchfreightEditProposal(clientObj).toPromise();
      {
      ...
      var data = await this.httpClientService.fetchfreightEditProposal(clientObj).toPromise();
      {
        this.dataList = data;

...
@Component({
  selector: 'app-discount-contractsummary',
  templateUrl: './discount-contractsummary.component.html',
  styleUrls: ['./discount-contractsummary.component.css'],
  standalone: false
})
export class DiscountContractsummaryComponent implements OnInit {
  serviceName;
  ...
  constructor(public dialogRef: MatDialogRef<DiscountContractsummaryComponent>, public dialog: MatDialog, private loaderService: LoaderService,
  ...
  async getDiscountDetails() {
    ...
    if (this.carrierName.toLowerCase() == 'ups') {
      ...
      var result = await this.httpClientService.fetchFreightRatesheet(userObj).toPromise();
      ...
      this.freightTargetSavingsListSignal.set(this.freightTargetSavingsList)
      var result = await this.httpClientService.fetchRatesheetLookUp(userObj).toPromise();
      ...
      var result: any = await this.fedexService.fetchFreightRatesheet(userObj).toPromise();
      this.dataSource = await result.filter((element: any) => element.criteria == "Current");
      if (this.targetIdList.length > 0) {
      ...
      this.freightTargetSavingsListSignal.set(this.freightTargetSavingsList)
      await this.fedexService.fetchRatesheetLookUp(userObj).toPromise().then(
        async (result: any) => {
          result.forEach((element: any, index: any) => {
            if (Number(element.field).toString() == 'NaN') {
              this.ZoneLookupList.push({ service: element.service, field: element.field, fieldVal: element.fieldVal, carrier: element.carrier.toLowerCase() })
  ...
  async getMinDetails() {
    ...
    if (this.carrierName.toLowerCase() == 'fedex') {
      ...
      var currentMinData = await this.fedexService.fetchMinDetailsPopup(clientObj).toPromise();
      ...
      for (let targetIndex = 0; targetIndex < this.freightTargetSavingsList.length; targetIndex++) {
        if (this.freightTargetSavingsList[targetIndex].targetId != null) {
          ...
          var targetMinData = await this.fedexService.fetchMinDetailsPopup(clientObj).toPromise();
      ...
      //set currentmin
      var currentMinData = await this.httpClientService.fetchMinDetailsPopup(clientObj).toPromise();
      var distinctServices = await currentMinData.filter(
      ...
      for (let targetIndex = 0; targetIndex < this.freightTargetSavingsList.length; targetIndex++) {
        if (this.freightTargetSavingsList[targetIndex].targetId != null) {
          ...
          clientObj['targetId'] = this.freightTargetSavingsList[targetIndex].targetId;
          var targetMinData = await this.httpClientService.fetchMinDetailsPopup(clientObj).toPromise();
          var distinctServices = await targetMinData.filter(
            (thing: any, i: any, arr: any) => arr.findIndex((t: any) => t.containerType == thing.containerType && Number(t.weightFrom) == Number(thing.weightFrom) && Number(t.weightTo) == Number(thing.weightTo)) === i
          );
          var zoneList = this.zoneLookupListService.filter((data: any) => data.carrier == this.freightTargetSavingsList[targetIndex].carrier.toLowerCase());
  ...
  delete() {
    ...
    alertDialog.afterClosed().subscribe(
      async result => {
        ...
        if (result == true) {
          if (this.carrierName.toLowerCase() == 'ups') {
            await this.httpClientService.deleteService(clientObj).toPromise().then(res2 => {
              if (res2 == true) {

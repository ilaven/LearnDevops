...
@Component({
  selector: 'app-accessorial-contractsummary',
  templateUrl: './accessorial-contractsummary.component.html',
  styleUrls: ['./accessorial-contractsummary.component.css'],
  standalone: false
})
export class AccessorialContractsummaryComponent implements OnInit {
  serviceName;
  ...
  service: string;
  themeoption: any;
  constructor(public dialogRef: MatDialogRef<AccessorialContractsummaryComponent>, public dialog: MatDialog, private loaderService: LoaderService,
  ...
  async getData() {
    ...
    if (this.carrierName.toLowerCase() == 'ups') {
      clientDetail['clientId'] = this.cookiesService.getCookie('clientId');
      this.httpclient.fetchAccLookUp(clientDetail).subscribe(async data => {
      ...
      if (targetIdList.length > 0) {
        for (let index = 0; index < targetIdList.length; index++) {
          ...
          var data = await this.httpclient.fetchAccLookUpTarget(clientDetail).toPromise();
          var targetData = {
      ...
      this.fedexService.fetchAccLookUp(clientDetail).subscribe(async (data: any) => {
        this.AccCurrentDetailsList = await data;
        this.dataSource = this.AccCurrentDetailsList;
        this.dataSourceSignal.set(this.dataSource);
      });
      if (targetIdList.length > 0) {
        for (let index = 0; index < targetIdList.length; index++) {
          clientDetail['targetId'] = targetIdList[index];
          var data = await this.fedexService.fetchAccLookUpTarget(clientDetail).toPromise();
          var targetData = {
            targetId: targetIdList[index],
            targetName: this.targetNameList[index],
            targetList: data,

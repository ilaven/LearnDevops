...
export class SchedularPopup implements OnInit, AfterViewInit {
  ...
  constructor(private dialogRef: MatDialogRef<SchedularPopup>, @Optional() @Inject(MAT_DIALOG_DATA) public data: any,
    private httpClientService: HttpClientService, private cookiesService: CookiesService, public dialog: MatDialog,
    private cdr: ChangeDetectorRef, private httpfedexService: HttpfedexService) {
  ...
  module1_creationCompleteHandlerFedex() {
    ...
    this.httpfedexService.fetchReportScheduler(t001ClientObj).subscribe({
      next: (descResult) => {

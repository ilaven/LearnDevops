...
import { FedexTrackingPopupComponent } from './tracking-popup/tracking-popup.component';
import { TrackingPopupFedexComponent } from '../../UPS/tracking/tracking-popup-fedex/tracking-popup-fedex.component';

@Component({
  selector: 'app-fedex-shipment-detail-search',
 ...
 * Create Component
 */
export class FedexShipmentDetailSearchComponent implements OnInit {
  ...
  constructor(private loaderService: LoaderService, private cookiesService: CookiesService,
    ...
    ;
    if (localStorage.getItem('payload_fedex')) {
      let searchUserobj: any = localStorage.getItem('payload_fedex');
  ...
  openTrackingFedExDialog() {
    const dialogRef = this.dialog.open(FedexTrackingPopupComponent, {
      width: '100%',
      height: '100%',

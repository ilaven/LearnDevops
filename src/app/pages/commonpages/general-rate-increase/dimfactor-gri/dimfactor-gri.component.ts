...
@Component({
  selector: 'app-dimfactor-gri',
  templateUrl: './dimfactor-gri.component.html',
  styleUrls: ['./dimfactor-gri.component.css'],
  standalone: false
})

export class DimfactorGRIComponent implements OnInit {
  serviceName: any;
  editable = false;
  ...
  constructor(public dialogRef: MatDialogRef<DimfactorGRIComponent>, private cd: ChangeDetectorRef, public dialog: MatDialog, private loaderService: LoaderService,
  ...
  async getProposalList(scenario: any) {
    ...
    if (this.selectedCarrier.carrierName.toLowerCase() == 'fedex') {
      ...
      var proposalData = await this.fedexService.fetchGRIfreightEditProposal(clientObj).toPromise();
      {
      ...
      var proposalData = await this.httpClientService.fetchGRIfreightEditProposal(clientObj).toPromise();
      {
        this.proposalList = proposalData;
  ...
  async save() {
    ...
    if (this.carrierName.toLowerCase() == 'ups') {
      ...
      this.httpClientService.saveOrUpdateDIMGRI(clientObj).subscribe((data: any) => {
        dimTableName = data[0];
      }
      );
    }
      ...
      clientObj['dimDetailsList'] = alteredList;
      var dimTableName: any;
      this.fedexService.saveOrUpdateDIMGRI(clientObj).subscribe((data: any) => {
        dimTableName = data[0];
      });
    }
    ...
    if (this.carrierName.toLowerCase() == 'ups') {
      ...
      var tableName = await this.httpClientService.saveOrUpdateDiscountsGRI(clientObj).toPromise();
      var targetObj: any = {};
      ...
      var result = await this.httpClientService.generateProposalDIMGRI(targetObj).toPromise();
    }
    else {
      clientObj['clientId'] = this.fedexClientId;;
      clientObj['clientName'] = this.selectedCarrier.clientName;
      ...
      clientObj['freightDetailsList'] = freightList;
      var tableName = await this.fedexService.saveOrUpdateDiscountsGRI(clientObj).toPromise();
      var targetObj: any = {};
      ...
      targetObj['tableName'] = tableName[0].runType;
      targetObj['dimTableName'] = dimTableName.runType;
      var result = await this.fedexService.generateProposalDIMGRI(targetObj).toPromise();
    }

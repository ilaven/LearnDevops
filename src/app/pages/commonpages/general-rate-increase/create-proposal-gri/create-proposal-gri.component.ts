...
import { AccessorialPopupGRIComponent } from "../accessorial-popup-gri/accessorial-popup-gri.component";
import { LoaderService } from "src/app/core/services/loader.service";
import { GRIdiscountComponent } from "../gri-discount/gri-discount.component";
import { GRIHWTAccountNumberUpdate } from "../hwt-accountnumber-update/hwt-accountnumber-update.component";

@Component({
  selector: 'app-create-proposal-gri',
  templateUrl: './create-proposal-gri.component.html',
  styleUrls: ['./create-proposal-gri.component.scss'],
  standalone: false
})
export class CreateProposalGRIComponent {
  ...
  constructor(public dialog: MatDialog, private cd: ChangeDetectorRef, private loaderService: LoaderService, public dialogRef: MatDialogRef<CreateProposalGRIComponent>, private httpClientService: HttpClientService, private cookiesService: CookiesService,
    private fedexService: HttpfedexService, @Optional() @Inject(MAT_DIALOG_DATA) public data: any, public resetDialog: MatDialogRef<ResetPopupGRIComponent>,
  ) {
    console.log(data);
    ...
    data.targetDetails.forEach((data: any) => this.targets.push(Object.assign({}, data)));
    if (data.valueClick == "create") {
      ...
      this.filteredScenario = this.targets.filter((data: any) => data.carrierName.toLowerCase() == this.carrierName.toLowerCase());
      this.filteredScenario.unshift(Object.assign({}, { targetId: 'current', targetName: 'Current Agreement' }));
      this.agreementYear = Number(this.carrierDetails.year);
    }
  ...
  async ngOnInit() {
    ...
    var accessorialServices = await this.httpClientService.fetchGRIAccessorialLookup(clientDetail).toPromise();
  ...
  async getTargetDetails() {
    ...
    if (this.targetDetails != '' && this.type != "create") {
      ...
      this.carrierName = this.targetDetails.carrierName;
      this.carrierNew = this.targetDetails.carrierName;
      this.agreementYear = Number(this.targetDetails.year);
      return this.targetDetails.targetId;
    }
  }
  ...
  async getData() {
    ...
    if (this.carrier.toLowerCase() == 'fedex') {
      ...
      var data = await this.fedexService.fetchGRIfreightEditProposal(clientObj).toPromise().catch((err) => {
        console.log(err);
        this.closeLoading();
      });
      {
      ...
      var dimData = await this.fedexService.fetchGRIDimFactor(clientObj).toPromise().catch((err) => {
        console.log(err);
        this.closeLoading();
      });
      ...
      this.accessorialDiscountList = await this.fedexService.fetchGRIAccDetailsPopup(clientObj).toPromise().catch((err) => {
        console.log(err);
        this.closeLoading();
      });
      ...
      var data = await this.httpClientService.fetchGRIfreightEditProposal(clientObj).toPromise().catch((err) => {
        console.log(err);
        this.closeLoading();
      });
      {
      ...
      var dimData = await this.httpClientService.fetchGRIDimFactor(clientObj).toPromise().catch((err) => {
        console.log(err);
        this.closeLoading();
      });
      ...
      this.dataSourceDIM = new MatTableDataSource(this.dimFactorList);
      this.dataSourceDIMSignal.set(this.dataSourceDIM);
      this.accessorialDiscountList = await this.httpClientService.fetchGRIAccDetailsPopup(clientObj).toPromise().catch((err) => {
        console.log(err);
        this.closeLoading();
      });
      var unsortedList = await this.accessorialDiscountList.filter(
  ...
  popdiscount(serviceName: string, subGroupName: string, listName: string, type: string) {
    ...
    const dialogRef = this.dialog.open(GRIdiscountComponent, {
      disableClose: true,
      width: '90vw',
      maxWidth: '80vw',
  ...
  popdiscountAcc(accName: any) {
    const dialogRef = this.dialog.open(AccessorialPopupGRIComponent, {
      disableClose: true,
      width: '90vw',
      maxWidth: '80vw',
      panelClass: this.panelClass,
  ...
  async saveTargetDetails() {
    ...
    if (this.currentAgreement) {
      ...
      if (this.carrier.toLowerCase() == 'ups') {
        ...
        var data = await this.httpClientService.saveOrUpdateTargetDetailsGRI(targetObj).toPromise().then(async res => {
          this.isSaved = true;
          message = "Agreement details saved successfully";

          if (Number(this.targetDetails.targetId) == 0) {
            await this.httpClientService.saveOrUpdateAgreementNoDetailsGRI(targetObj).toPromise().then(res2 => {
        ...
        var data = await this.fedexService.saveOrUpdateTargetDetailsGRI(targetObj).toPromise().then(async res => {
          this.isSaved = true;
          message = "Agreement details saved successfully";
          if (Number(this.targetDetails.targetId) == 0) {
            await this.httpClientService.saveOrUpdateAgreementNoDetailsGRI(targetObj).toPromise().then(res2 => {
  ...
  async nextbtn_clickHandler(event: any) {
    ...
    if (this.buttonName == 'Generate' && this.type.toLowerCase() == 'create' && this.currentAgreement) {
      if (this.carrier.toLowerCase() == 'ups') {
        ...
        await this.httpClientService.saveOrUpdateAgreementNoDetailsGRI(targetObj).toPromise().then(res => {
          this.agreementNoDuplicate = this.agreementNo;
          this.isSaved = true;
        }).catch((err) => {
        });
      ...
      if (this.carrier.toLowerCase() == 'ups') {
        ...
        await this.httpClientService.saveOrUpdateTargetDetailsGRI(targetObj).toPromise().then(res => {
          this.isSaved = true;
        }).catch((err) => {
          console.log(err);
          this.closeLoading();
        ...
        await this.fedexService.saveOrUpdateTargetDetailsGRI(targetObj).toPromise().then(res => {
          this.isSaved = true;
        }).catch((err) => {
          console.log(err);
          this.closeLoading();
    ...
    if (this.buttonName == 'Generate') {
      ...
      if (this.type.toLowerCase() == 'edit') {
        ...
        if (this.carrier.toLowerCase() == 'ups') {
          ...
          await this.httpClientService.saveOrUpdateDiscountsGRI(clientObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
        }
          ...
          clientObj['freightDetailsList'] = alteredList;
          await this.fedexService.saveOrUpdateDiscountsGRI(clientObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
        }
      ...
      if (this.type.toLowerCase() == 'edit') {
        ...
        if (this.carrier.toLowerCase() == 'ups') {
          ...
          clientObj['freightDetailsList'] = alteredList;
          await this.httpClientService.saveOrUpdateDiscountsHWTGRI(clientObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
        }
          ...
          await this.fedexService.saveOrUpdateDiscountsHWTGRI(clientObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
        }
      ...
      if (this.buttonName == 'Generate') {
        ...
        if (this.carrierName.toLowerCase() == 'fedex') {
          ...
          await this.httpClientService.saveOrUpdateAccountNumberGRI(clientObj).toPromise();
        }
      }
      ...
      if (this.type.toLowerCase() == 'edit') {
        ...
        if (this.carrier.toLowerCase() == 'ups') {
          ...
          this.httpClientService.saveOrUpdateDIMGRI(clientObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
        }
          ...
          clientObj['dimDetailsList'] = alteredList;
          this.fedexService.saveOrUpdateDIMGRI(clientObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
        }
      ...
      if (this.type.toLowerCase() == 'create' && this.buttonName == 'Generate') {
        ...
        if (this.carrier.toLowerCase() == 'ups') {
          ...
          this.dimTable = await this.httpClientService.saveOrUpdateDIMGRI(clientObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
          ...
          this.dimTable = await this.fedexService.saveOrUpdateDIMGRI(clientObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
        }
      ...
      if (this.type.toLowerCase() == 'edit') {
        ...
        if (this.carrier.toLowerCase() == 'ups') {
          ...
          this.httpClientService.saveOrUpdateAccDiscountsGRI(clientObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
        }
          ...
          clientObj['accDetailsList'] = alteredList;
          this.fedexService.saveOrUpdateAccDiscountsGRI(clientObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
        }
      ...
      if (this.type.toLowerCase() == 'create' && this.buttonName == 'Generate') {
        ...
        if (this.carrier.toLowerCase() == 'ups') {
          ...
          clientObj['accDetailsList'] = alteredList;
          await this.httpClientService.saveOrUpdateAccDiscountsGRI(clientObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
        }
          ...
          clientObj['accDetailsList'] = alteredList;
          await this.fedexService.saveOrUpdateAccDiscountsGRI(clientObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
        }
    ...
    if (this.buttonName == 'Generate') {
      if (this.carrier.toLowerCase() == 'ups') {
        ...
        if (this.currentAgreement && this.dimChanged) {
          ...
          type = await this.httpClientService.generateProposalGRI(targetObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
        }
        ...
        if (this.currentAgreement && this.dimChanged) {
          ...
          type = await this.fedexService.generateProposalGRI(targetObj).toPromise().catch((err) => {
            console.log(err);
            this.closeLoading();
          });
        }
  ...
  fromWeightFocusout(value: any, columnName: any, rowNumber: any, type: any) {
    ...
    if ((this as any)[tableList][rowNumber - 1] != undefined && (this as any)[tableList][rowNumber - 1].subGroup == subGroup) {
      ...
      if ((Number(value) <= toWeight || Number(value) > (toWeight + 1) || Number(value).toString() == 'NaN') && Number(value) != 0) {
    ...
    // ALWAYS validate internal range
    if (weightFrom > weightTo) {
      if (weightTo != 0) {
        (this as any)[tableList][rowNumber][columnName] = weightTo;
        (this as any)[minMaxList][rowNumber].minWeight = weightTo;

        if ((this as any)[tableList][rowNumber].isNewRow == undefined) {
          var alteredObject = (this as any)[listName].filter((data: any) => data.subGroup == subGroup && data.weightFrom == weightFrom);
          alteredObject.forEach((element: any) => {
            element[columnName] = weightTo;
          });
        }
  ...
  async toWeightFocusout(value: any, columnName: any, rowNumber: any, type: any) {
    ...
    if ((this as any)[tableList][rowNumber + 1] != undefined && (this as any)[tableList][rowNumber + 1].subGroup == subGroup) {
      ...
      if ((this as any)[tableList][rowNumber].isNewRow == undefined) {
        var alteredObject = (this as any)[listName].filter((data: any) => data.subGroup == subGroup && data.weightTo == weightTo);
        alteredObject.forEach((element: any) => {
          element.weightTo = weightFrom;
        });
      }
      (this as any)[tableList][rowNumber].weightTo = weightFrom;
      (this as any)[minMaxList][rowNumber].maxWeight = weightFrom;
      this.toWeightToolTipId = rowNumber;
      setTimeout(() => {
        this.toWeightToolTipId = -1;
      }, 2000)
  ...
  async deleteRowFRTRatesheet(frtRateObjList: any) {
    ...
    if (this.carrier.toLowerCase() == 'ups') {
      ...
      await this.httpClientService.deleteFRTRatesheetGRI(deleteFRTRateObj).toPromise().catch((err) => {
        console.log(err);
        this.closeLoading();
      });
    }
  ...
  deleteRowDIM(dimObj: any) {
    ...
    if (this.carrier.toLowerCase() == 'ups') {
      this.httpClientService.deleteDIMGRI(deleteDIMObj).subscribe((data: any) => { console.log('deleted successfully.') });
    }
    else {
      this.fedexService.deleteDIMFedExGRI(deleteDIMObj).subscribe((data: any) => { console.log('deleted successfully.') });
    }
  }
  async setRowSpan() {
  ...
  accountNumberUpdate(service: any, serviceName: any) {
    var hwtDialog = this.dialog.open(GRIHWTAccountNumberUpdate, {
      width: "86%",
      height: "86%",
      panelClass: this.panelClass,
      data: {
    ...
    hwtDialog.afterClosed().subscribe(async data => {
      await this.setMinMaxHWT();
    });
  }
  ...
  delete() {
    ...
    this.httpClientService.deleteTargetDetailsGRI(param).subscribe(response => {
      let proposalName = "";
      for (let index = 0; index < this.targetList.length; index++) {
        if (this.targetList[index].targetId == this.proposalName) {
          proposalName = this.targetList[index].targetName;
  ...
  async changeAgreementYear(event: any) {
    this.agreementYear = Number(event.value.toString());
  }

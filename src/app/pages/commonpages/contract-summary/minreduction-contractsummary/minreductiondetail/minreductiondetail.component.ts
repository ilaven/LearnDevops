...
@Component({
  selector: 'app-minreductiondetail',
  templateUrl: './minreductiondetail.component.html',
  styleUrls: ['./minreductiondetail.component.css'],
  standalone: false
})
export class MinreductiondetailComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<MinreductiondetailComponent>, private cd: ChangeDetectorRef, public dialog: MatDialog, private loaderService: LoaderService,
  ...
  async getNetMinDetails() {
    ...
    if (this.carrier.toLowerCase() == 'fedex') {
      ...
      var currentNetMinData = await this.fedexService.fetchMinNetdetailsPopup(clientObj).toPromise();
      ...
      clientObj['clientId'] = this.cookiesService.getCookieItem('clientId');
      clientObj['service'] = this.serviceName;
      var currentNetMinData = await this.httpClientService.fetchMinNetdetailsPopup(clientObj).toPromise();
      var distinctServices = await currentNetMinData.filter(
        (thing: any, i: any, arr: any) => arr.findIndex((t: any) => t.subGroup == thing.subGroup) === i
      );
      var zoneList = this.zoneLookupList.filter((data: any) => data.carrier == this.carrierName.toLowerCase());
  ...
  async fetchRateSheetLookUpFedEx(userObj: any) {
    ...
    var result = await this.fedexService.fetchRatesheetLookUp(userObj).toPromise();

...
export class ContractsummarydiscountComponent implements OnInit {
  ...
  async fromWeightFocusout(value: any, columnName: any, containerType: any, rowNumber: any, type: any) {
    ...
    if (value === '' || value === null || value === undefined) {
      ...
      for (let i = rowNumber - 1; i >= 0; i--) {
        if (currentList[i].containerType.toLowerCase() === containerTypeStr) {
          prevRowIndex = i;
          break;
        }
      }
      if (prevRowIndex !== -1) {
        var prevRow = currentList[prevRowIndex];
    ...
    if (finalValue !== oldWeightFrom) {
      ...
      // Scenario 2: Auto calculate Weight To if it's empty or system-driven
      if (finalValue !== '' && finalValue !== null) {
        let shouldUpdateTo = false;
        if (weightTo === '') {
          shouldUpdateTo = true;
        } else if (oldWeightFrom !== '' && oldWeightFrom !== null && Number(weightTo) === Number(oldWeightFrom) + 1) {
          shouldUpdateTo = true;
        }

        if (shouldUpdateTo) {
          let newTo = Number(finalValue) + 1;
          await this.toWeightFocusout(newTo, 'weightTo', row.containerType, rowNumber, type);
          return;
        } else {
          // Validation Logic: If From > To, show validation error
          let prevToData: any = Number(finalValue);
          if (weightTo !== '' && prevToData > Number(weightTo)) {
  ...
  async toWeightFocusout(value: any, columnName: any, containerType: any, rowNumber: any, type: any) {
    ...
    // Scenario 1: Default Behavior on Focus Out
    if (finalValue === '' || finalValue === null || finalValue === undefined) {
      if (weightFrom !== '' && weightFrom !== null) {
        finalValue = Number(weightFrom) + 1;
      }
    }
    ...
    if (finalValue !== oldWeightTo) {
      ...
      // Validation Logic: If To < From, show validation error
      let finalValueData: any = Number(finalValue);
      if (finalValue !== '' && weightFrom !== '' && finalValueData < Number(weightFrom)) {
        ...
        if (this.serviceType.toLowerCase() !== 'intl' && finalValue !== '' && finalValue !== null) {
          ...
          if (nextRowIndex !== -1) {
            ...
            // Scenario 3: Auto-calculation applies only when fields are empty or system-driven
            if (nextRow.weightFrom === '' || nextRow.weightFrom === null || (oldWeightTo !== '' && oldWeightTo !== null && Number(nextRow.weightFrom) === Number(oldWeightTo) + 1)) {

import { Component, Inject, Optional, OnInit, OnDestroy, NgZone } from '@angular/core';
...
@Component({
  selector: 'app-dhl-more-service-popup',
  templateUrl: './dhl-more-service-popup.component.html',
  standalone: false
})
export class DhlMoreServicePopupComponent implements OnInit, OnDestroy {
  private pie_FrtChart!: am4charts.PieChart;
  private pie_AccessorialChart!: am4charts.PieChart;
  private _frtSub!: Subscription;
  private _accessorialSub!: Subscription;
  month: any;
  year: any;
  fromPage;
  invoiceMonth;
  ...
  panelClass: any;
  constructor(
    public dialogRef: MatDialogRef<DhlMoreServicePopupComponent>,
  ...
  fetch_FrtPiechart_MoreService() {
    this.moreviewServiceFormGroup.get('chargeType')?.setValue('FRT');
    this._frtSub = this.httpDhlService.fetchFRTServices(this.moreviewServiceFormGroup.value)?.subscribe(
      result => {
  ...
  fetch_AccessorialPiechart_MoreService() {
    this.moreviewServiceFormGroup.get('chargeType')?.setValue('ACC');
    this._accessorialSub = this.httpDhlService.fetchFRTServices(this.moreviewServiceFormGroup.value)?.subscribe(
      result => {
  ...
  async createSeriesFromAC_for_FrtPieChart(collection: any) {
    ...
    this.zone.runOutsideAngular(() => {
      if (this.pie_FrtChart) { this.pie_FrtChart.dispose(); }
  ...
  async createSeriesFromAC_for_AccessorialPieChart(collection: any) {
    ...
    this.zone.runOutsideAngular(() => {
      if (this.pie_AccessorialChart) { this.pie_AccessorialChart.dispose(); }
  ...
  async reportExcelDownload(event: any) {
    ...
    var clientName = this.moreviewServiceFormGroup.get('clientName')?.value;
    var clientId = this.moreviewServiceFormGroup.get('clientId')?.value;
    var weightchargetype = this.dashBoardSHP.get('chargetypevalue')?.value;
    var accountNumber = this.moreviewServiceFormGroup.get('accountNumber')?.value;
  ...
  ngOnDestroy() {
    this._frtSub?.unsubscribe();
    this._accessorialSub?.unsubscribe();
    this.zone.runOutsideAngular(() => {

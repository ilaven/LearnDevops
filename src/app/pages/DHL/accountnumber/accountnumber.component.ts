...
@Component({
  selector: 'app-dhl-accountnumber',
  templateUrl: './accountnumber.component.html',
  styleUrls: ['./accountnumber.component.scss'],
  standalone: false
})
export class DhlAccountnumberComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async fetchDhlAccountDetails(clientIdUps: any) {
    ...
    try {
      const result = await firstValueFrom(
        this.httpDhlService.fetchaccountDetails(this.t002ClntObjUps)
      );
      this.accountACDhl = result;
      if (!this.accountACDhl || this.accountACDhl.length === 0) {
  ...
  // UI -Logic 
  SortOrder_Update_ClickHandlerDHL() {
    this.httpDhlService.insertOrUpdateAccountSortOrder(this.accountAC).subscribe({
      next: (result: any) => {
        if (result == true)
  ...
  accountUpdate_ClickHandlerDHL(rowObj: any, index: any) {
    var updateObj = {};
    updateObj = rowObj;
    this.httpDhlService.updateAccountNumber(updateObj).subscribe({
      next: (result: any) => {

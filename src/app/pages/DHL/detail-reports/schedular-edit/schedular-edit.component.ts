...
@Component({
  selector: 'app-dhl-schedular-edit',
...
export class SchedularEditComponent implements OnInit {
  ...
  btn_submit_click(): void {
    ...
    if (this.btnEdit === 'yes') {
      ...
      this.httpDhlService.updateReportScheduler(t007reportLogObj).subscribe({
        next: (result) => {
      ...
      this.httpDhlService.addMailId(t007reportLogObj).subscribe({
        next: (result) => {
          if (result) {
            this.openModal(

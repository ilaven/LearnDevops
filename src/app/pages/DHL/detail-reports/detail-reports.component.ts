...
import { DhlReportLogComponent } from './report-log/report-log.component';
...
@Component({
  selector: 'app-dhl-detail-reports',
  templateUrl: './detail-reports.component.html',
  styleUrls: ['./detail-reports.component.scss'],
  standalone: false
})
export class DhlDetailReportsComponent implements OnInit, AfterViewInit {
  ...
  constructor(private loaderService: LoaderService, private cookiesService: CookiesService,
    ...
    private httpDhlService: HttpDhlService, public datePipe: DatePipe,
    ...
    try {
      ...
      await this.commonService.getUserprofileData().then(
        (result: any) => {
          ...
          this.detailReportsFormGroup.get('dhlClientId')?.setValue(this.clientID);
  ...
  fetchaccountDetails(t002AccSrchObj: any, isInitial = false): void {
    this.httpDhlService.fetchaccountDetails(t002AccSrchObj).subscribe({
      next: (result: any[]) => {
  ...
  collection(isInitial = false): void {
    if (!isInitial) this.openLoading();
    var t001ClientObj = {};
    this.httpDhlService.fetchReportDescription(t001ClientObj).subscribe({
      next: (result: any) => {
        this.ReportDescriptionResult(result);

      }, error: error => {
        this.closeLoading(); console.error(error)
      }
    });
  }
  ...
  async reportLog_clickHandler() {
    ...
    await this.httpDhlService.fetchReportLog(userprofile).subscribe({
      next: (result: any[]) => {
        var reportDetails = result;
        const dialogRef = this.dialog.open(DhlReportLogComponent, {
          width: '95%',
          // height: '50%',
          // panelClass: this.panelClass,
  ...
  report_clickHandler(event: any): void {
    ...
    if (pdf_btnselected == 'Stripped Data') {
      ...
      if (reportsselectedValue == "LANE_SUMMARY_REPORT") {
        this.detailReportsFormGroup.get('reportName')?.setValue("Lane Summary Report");
      }
    ...
    if (this.detailReportsFormGroup.get('reportType')?.value == "INVOICE_STRIPPED_DATA" || this.detailReportsFormGroup.get('reportType')?.value == "INVOICE_DATA") {
      ...
      if (this.clientName == "admin") {
        ...
        this.httpDhlService.runReport(this.detailReportsFormGroup.value).subscribe({
          next: (result: any) => {

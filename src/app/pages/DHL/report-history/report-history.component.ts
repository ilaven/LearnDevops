...
@Component({
  selector: 'app-dhl-report-history',
 ...
 * Create Component
 */
export class DHLReportHistoryComponent implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  constructor(private loaderService: LoaderService, private cookiesService: CookiesService,
    private httpDhlService: HttpDhlService, public dialog: MatDialog, private router: Router, private commonService: CommonService,
  ...
  reportLog_clickHandler(param: any) {
    ...
    this.httpDhlService.fetchReportLog(userprofile).subscribe({
      next: result => {
        this.closeLoading();

...
import { DhlTrackingPopupComponent } from './tracking-popup/tracking-popup.component';
...
@Component({
  selector: 'app-dhl-shipment-detail-search',
 ...
 * Create Component
 */
export class DhlShipmentDetailSearchComponent implements OnInit {
  ...
  constructor(private loaderService: LoaderService, private cookiesService: CookiesService,
    ...
    private datePipe: DatePipe, private cd: ChangeDetectorRef, private httpDhlService: HttpDhlService
  ) {
    ...
    this.cookiesService.carrierType.subscribe((clienttype) => {
      ...
      if (this.clientType != "Dhl") {
        this.router.navigate(['/dashboard/dashboard']);
      }
    });
    if (localStorage.getItem('payload_dhl')) {
      let searchUserobj: any = localStorage.getItem('payload_dhl');
  ...
  async fetch_Trakingnumber(searchUserobjValue: any) {
    ...
    this.openLoading();
    await this.httpDhlService.fetchShipmentDetailSearch(searchUserobjValue).subscribe(
      (result: any) => {
  ...
  async getTrackingDetail(trackingParam: any) {
    var dateFr = this.searchForm.get('fromdate')?.value;
    var dateFrom = this.datePipe.transform(dateFr, "yyyy-MM-dd");
    var dateT = this.searchForm.get('fromdate')?.value;
    var dateTo = this.datePipe.transform(dateT, "yyyy-MM-dd");
  ...
  openDialog() {
    const dialogRef = this.dialog.open(DhlTrackingPopupComponent, {
      width: '100%',
      height: '100%',
  ...
  generatetrackingexcel(trackingNo: any, clientBillingId: any) {
    ...
    t007_reportlogobj['clientId'] = this.clientID;
    t007_reportlogobj['clientname'] = this.clientName;
    t007_reportlogobj['crmaccountNumber'] = clientBillingId; //"T004_" + this.clientName + "_" + this.datePipe.transform(dateT, "yyyy");
    t007_reportlogobj['login_id'] = this.loginId.toString();
    t007_reportlogobj['fzone'] = 0;
    t007_reportlogobj['tzone'] = 0;
    this.httpDhlService.runReport(t007_reportlogobj).subscribe(
  ...
  generatetrackingpdf(trackingNo: any, clientBillingId: any) {
    ...
    Master_reportlogobj["clientBillingId"] = clientBillingId;

...
@Component({
  selector: 'app-ontrac-charge-description',
  templateUrl: './charge-description.component.html',
  styleUrls: ['./charge-description.component.scss'],
  standalone: false
})
export class OnTracChargeDescriptionComponent implements OnInit, AfterViewInit {
  // Scroll references
  ...
  ngOnInit(): void {
    ...
    // Set default txt_lbl to prevent ExpressionChangedAfterItHasBeenCheckedError
    this.txt_lbl = 'Advance Charge Description Freight and Accessorial Separated';

    // Initialize columns with default structure to prevent ExpressionChangedAfterItHasBeenCheckedError
    this.columns = [
      { field: 'Group', fieldVal: 'group', label: 'Group', visible: true, index: 0 },
      { field: 'Type', fieldVal: 'type', label: 'Type', visible: true, index: 1 },
      { field: 'Net Charge', fieldVal: 'netamount', label: 'Net Charge', visible: true, index: 2 },
      { field: 'Number of Packages', fieldVal: 'nooftrackingno', label: 'Number of Packages', visible: true, index: 3 },
      { field: 'Avg.Cost', fieldVal: 'avgcost', label: 'Avg.Cost', visible: true, index: 4 },
      { field: 'Total Weight', fieldVal: 'totalweight', label: 'Total Weight', visible: true, index: 5 },
      { field: '% of Spend', fieldVal: 'percentofspent', label: '% of Spend', visible: true, index: 6 }
    ];
    ...
    // Check if the years are different, then set same years
  ...
  async fetchaccountDetails(): Promise<void> {
    ...
    this.httpOntracService.fetchaccountDetails(this.apiControllerFormGroup.value).subscribe({
      next: result => {
  ...
  async fetchAdvanceChargeDescriptionDetails(): Promise<void> {
    ...
    await this.httpOntracService.fetchChargeDescriptionDetails(this.apiControllerFormGroup.value).subscribe({
      next: (result) => {
  ...
  checkerMethod(fetchChargeDescriptionDetails: any[]): void {
    ...
    for (let loop = 0; loop < this.fetchChargeDescriptionDetails.length; loop++) {
      if (this.fetchChargeDescriptionDetails[loop]['type'] == "Accessorial") {
        if (this.fetchChargeDescriptionDetails[loop]['costperpound'])
          this.fetchChargeDescriptionDetails[loop]['costperpound'] = "";
        if (this.fetchChargeDescriptionDetails[loop]['totalweight'])
          this.fetchChargeDescriptionDetails[loop]['totalweight'] = "";
        if (this.fetchChargeDescriptionDetails[loop]['avgweight'])
          this.fetchChargeDescriptionDetails[loop]['avgweight'] = "";
      }
    }
  ...
  async fetchData_ACAS(resultParameter: any[]): Promise<void> {
    ...
    for (let count = 0; count < resultParameter.length; count++) {
      if (resultParameter[count].type == "Freight") {
        sum += Number(resultParameter[count].netamount);
        this.freightNetAmount = sum;
        if (resultParameter[count].group.indexOf("Miscellaneous (Z)") < 0 && resultParameter[count].group.indexOf("Service Fee (B)") < 0) {
          this.freightNoofShipment += Number(resultParameter[count].nooftrackingno);
        }

        if (resultParameter[count].airground == "Ground") {
          this.ground += Number(resultParameter[count].netamount);
          this.groundFRTGround += Number(resultParameter[count].nooftrackingno);
        }
        else if (resultParameter[count].airground == "Air") {
          this.air += Number(resultParameter[count].netamount);
          this.airFRTGround += Number(resultParameter[count].nooftrackingno);

        }
      }
      else if (resultParameter[count].type == "Accessorial") {
        if (resultParameter[count].group.indexOf("Saturday Charge") < 0) {
  ...
  setDisplayedColumns() {
    ...
    if (reportName == "Freight_Accessorial_Separated") {
      this.columns = [
        ...
        //{ field: 'Cost Per Pound', fieldVal: 'costperpound', label: 'Cost Per Pound', visible: false },
        ...
        // { field: 'Avg.Weight', fieldVal: 'avgweight', label: 'Avg.Weight', visible: false }
      ];
    ...
    for (let loop1 = 0; loop1 < displayedColumns.length; loop1++) {
      for (let loop2 = 0; loop2 < this.fetchChargeDescriptionDetails.length; loop2++) {
        if (displayedColumns[loop1] == 'netamount' || displayedColumns[loop1] == 'avgcost' || displayedColumns[loop1] == 'costperpound') {
          ...
          displayedColumns[loop1] == 'avgweight') {
        ...
        next: (result) => {
          ...
          if (serviceType == 'Freight_Accessorial_Combined') {
            for (let loop = 0; loop < result.length; loop++) {
              if (result[loop]['type'] == "Freight + Accessorial")
                result[loop]['Typefrt_frtAcc'] = "Freight + Accessorial";
              result[loop]['type'] = "Freight";
  ...
  async ExportTOExcel(): Promise<void> {
    ...
    if (reportName == "Freight_Accessorial_Separated") {
      t007_reportlogobj["reportType"] = "FreightandAccessorialSeparated_Details";
      t007_reportlogobj["reportName"] = "Freight and Accessorial Separated Details";
      t007_reportlogobj["designFileName"] = "FreightandAccessorialSeparated_Details";
    }
    if (reportName == "Freight_Accessorial_Combined") {
      t007_reportlogobj["reportType"] = "FreightandAccessorialCombined_Details";
      t007_reportlogobj["reportName"] = "Freight and Accessorial Combined Details";
  ...
  generateExcel() {
    ...
    if (this.reportNameSearched == 'Freight_Accessorial_Separated') {
      fileNameSecond = "_Freight_Accessorial_Seperated_";
      title = 'Freight Accessorial Seperated';
    }
        ...
        if (typeVal == "Freight") {
          ...
          row.eachCell((cell: any, number: any) => {
            ...
            if (number == 3 || number == 5) {
              worksheet.getCell("'" + this.GetExcelColumnName(number) + "" + (rowNum) + "'").numFmt = '$#,##0.00';
            }
            else if (number == 4) {
              worksheet.getCell("'" + this.GetExcelColumnName(number) + "" + (rowNum) + "'").numFmt = '#,##0';
            }
            else if (number == 6) {
              worksheet.getCell("'" + this.GetExcelColumnName(number) + "" + (rowNum) + "'").numFmt = '#,##0.00';
            }

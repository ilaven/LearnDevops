...
@Component({
  selector: 'app-ontrac-report-log',
 ...
 * Create Component
 */
export class OnTracReportLogComponent implements OnInit {
  ...
  constructor(private loaderService: LoaderService, private cookiesService: CookiesService,
    private httpOntracService: HttpOntracService, public dialog: MatDialog, private commonService: CommonService, public dialogRef: MatDialogRef<OnTracReportLogComponent>,
  ...
  loadUser() {
    this.httpOntracService.fetchReportLog(this.clientProfile).subscribe({
      next: (result) => {

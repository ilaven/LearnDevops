...
@Component({
  selector: 'app-ontrac-schedular-edit',
...
export class SchedularEditComponent implements OnInit {
  ...
  btn_submit_click(): void {
    ...
    if (this.btnEdit === 'yes') {
      ...
      this.httpOntracService.updateReportScheduler(t007reportLogObj).subscribe({
        next: (result) => {
      ...
      this.httpOntracService.addMailId(t007reportLogObj).subscribe({
        next: (result) => {
          if (result) {
            this.openModal(
